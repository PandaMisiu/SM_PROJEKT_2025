...
std::ofstream zapiszDoPliku(Uint16 szerokoscObrazka, Uint16 wysokoscObrazka, Uint8 tryb, Uint8 dithering,
                            Uint8 kompresja) {
...
std::ifstream odczytPliku(Uint16& szerokoscObrazka, Uint16& wysokoscObrazka, Uint8& tryb, Uint8& dithering,
                          Uint8& kompresja) {
    char identyfikator[] = "13";
    ...
    for (int y = 0; y < wysokosc / 2; y++) {
        for (int x = 0; x < szerokosc / 2; x++) {
            if (colorType == RGB555_TYPE) {
                ...
                setPixel(x + szerokosc / 2, y, GRAY_tab[index], GRAY_tab[index], GRAY_tab[index]);
        ...
        for (int bx = 0; bx < liczbaBlokowX; bx++) {
            ...
            for (int yy = 0; yy < rozmiarBloku; yy++) {
                for (int xx = 0; xx < rozmiarBloku; xx++) {
                    SDL_Color rgb;
                    if (colorType == RGB888_TYPE) {
                        rgb = {odtworzonyBlok_A.dane[xx][yy], odtworzonyBlok_B.dane[xx][yy],
                               odtworzonyBlok_C.dane[xx][yy]};
                        setPixel(bx * rozmiarBloku + xx + szerokosc / 2, by * rozmiarBloku + yy, rgb.r, rgb.g, rgb.b);
                    } else if (colorType == YCbCr_TYPE) {
                        YCbCr ycbcr = YCbCr(odtworzonyBlok_A.dane[xx][yy], odtworzonyBlok_B.dane[xx][yy],
                                            odtworzonyBlok_C.dane[xx][yy]);
            ...
            for (int yy = 0; yy < rozmiarBloku; yy++) {
                for (int xx = 0; xx < rozmiarBloku; xx++) {
                    ...
                    if (colorType == RGB555_TYPE) {
                        ...
                        setRGB555(bx * rozmiarBloku + xx + szerokosc / 2, by * rozmiarBloku + yy, GRAY, GRAY, GRAY);
                    } else {
                        setPixel(bx * rozmiarBloku + xx + szerokosc / 2, by * rozmiarBloku + yy, GRAY, GRAY, GRAY);
                    }
                }
            }
        }
